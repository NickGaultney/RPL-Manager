...
class PrintersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create printer" do
    assert_difference('Printer.count') do
      post printers_url, params: { printer: { name: @printer.name, status: @printer.status } }
    end
  ...
  test "should update printer" do
    patch printer_url(@printer), params: { printer: { name: @printer.name, status: @printer.status } }
